...
class Game2048 extends BaseView {
    ...
    private gridSpacing: number = 20*Layout.getInstance().scale;

    private gridRadius: number = 12*Layout.getInstance().scale;

	private gridSize: number;

    private newGame:eui.Button

    private newGameInGameOver:eui.Label;

    private slidIn2048:egret.tween.TweenGroup;
    ...
    private blockContent:number[] = [2,4,8,16,32,64,128,256,512,1024,2048,4096,8192];

    private nums = DataManager.getInstance().gameVO.initDataArr;
    // private nums = 
    // [
    //     {
    //         "num": 2,
